...
import { ArrowUp, Paperclip, Square, X, StopCircle, Mic, Globe, BrainCog, FolderCode, Zap } from "lucide-react";
...
interface PromptInputBoxProps {
  onSend?: (message: string, files?: File[], useStreaming?: boolean) => void;
  isLoading?: boolean;
  placeholder?: string;
  className?: string;
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend = () => { }, isLoading = false, placeholder = "Type your message here...", className, defaultStreaming = true, autoFocus } = props;
  ...
  const [showStreaming, setShowStreaming] = React.useState(defaultStreaming);
  ...
  const handleSubmit = () => {
    if (input.trim() || files.length > 0) {
      ...
      onSend(formattedInput, files, showStreaming);
      setInput("");
      setFiles([]);
      setFilePreviews({});
    }
