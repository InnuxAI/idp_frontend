...
// RAG Chat specific components
import { RagChatPanel } from "@/components/rag-chat/RagChatPanel";
import { RagDocumentPanel } from "@/components/rag-chat/RagDocumentPanel";
import { RagUploadModal } from "@/components/rag-chat/RagUploadModal";
import { RagSearchDropdown } from "@/components/rag-chat/RagSearchDropdown";
import { RagUploadNotifications } from "@/components/rag-chat/RagUploadNotifications";
import { ragApi, RagDocument } from "@/lib/rag-api";

// Panel animation variants – identical to Zete for visual consistency
const panelVariants = {
    hidden: {
        opacity: 0,
        scale: 0.95,
...
function RagChatPageContent() {
    ...
    // Load a document when user selects from search
    const handleSelectDocument = useCallback(async (documentId: string) => {
        try {
            ...
            console.error("[rag-chat] handleSelectDocument error:", err);
        }
    }, []);
    ...
    return (
        ...
        >
            ...
            <SidebarInset className="flex flex-col h-screen overflow-hidden dark:bg-zinc-950 dark:text-zinc-100 relative">
                ...
                <header className="h-14 shrink-0 flex items-center justify-between gap-2 px-4 dark:border-zinc-800">
                    <div className="flex items-center gap-2">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList>
                                ...
                                >
                                    <BreadcrumbItem>
                                        <BreadcrumbPage className="dark:text-zinc-100">RAG Chat</BreadcrumbPage>
                                    </BreadcrumbItem>
                                </motion.div>
                            </BreadcrumbList>
                        </Breadcrumb>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <motion.button
                            ...
                            whileTap={{ scale: 0.95 }}
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-violet-600 hover:bg-violet-700 text-white shadow-md shadow-violet-500/20 transition-colors"
                        >
                            <IconUpload size={14} />
                        ...
                        {/* Search Dropdown */}
                        <RagSearchDropdown onSelectDocument={handleSelectDocument} />
                ...
                >
                    ...
                    <AnimatePresence mode="popLayout">
                        {showChat && (
                            ...
                            >
                                <RagChatPanel onSourceClick={(docId) => {
                ...
                <RagUploadNotifications onAllComplete={handleUploadComplete} />
            </SidebarInset>
        </SidebarProvider>
    );
}
